...
class OptionParameter
{
    ...
    protected string $name = self::OPTION_NAME;
    ...
    {
        if (!is_array(self::$data[$this->moduleID][$this->name])) {
            $data = Option::get(
                        $this->moduleID,
                        $this->name,
                        false, \CSite::GetDefSite()
                    );
            self::$data[$this->moduleID][$this->name] = $data ? json_decode($data, true) : [];
        }
        return self::$data[$this->moduleID][$this->name];
    }
